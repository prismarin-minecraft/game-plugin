 ...
 * Written by Maga
 **/
public class SetRespawnSubCommand extends HelpSubCommand<Player> {

    private final KitService service;

    public SetRespawnSubCommand(KitService service) {
        super("setRespawn");
        this.service = service;
        setUsage("<id> <respawn>");
        setDescription("Change if kit is a respawn kit");
    ...
    public boolean send(Player player, SpigotArguments arguments) throws CommandException {
        if(arguments.getLength() >= 3) {
            ...
            kit.setRespawn(respawn);
            service.getRepository().save(kit);
            player.sendMessage(PrismarinConstants.PREFIX + "§7You successfully changed the §8'§arespawn§8' §7value of kit §a" + id);
            return true;
        }
        return false;
    }
